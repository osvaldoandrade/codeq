...
description: |
  ...
  a comprehensive project plan. Creates or updates a planning issue with prioritized
...
safe-outputs:
  # create-discussion: # needed to create the project plan issue
...
1. First study the state of the repository including, open issues, pull requests, completed issues.
   ...
   1a. As part of this, look for the open issue with title starting with "${{ github.workflow }}", which is the existing project plan. Read the plan, and any comments on the plan. If no such issue exists, ignore this step.
